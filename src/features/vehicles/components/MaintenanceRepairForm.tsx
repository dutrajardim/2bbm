  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await db.vehicleMaintenances.add({
        id: uuid(),
        datetime: Date.now(),
        type: repairType as "REPARO_COMPLETO" | "ATUALIZACAO_DE_REPARO",
        plateNumber: data.plate,
        reasonDescription: data.description,
        name: data.name,
        disabled: repairType === 'ATUALIZACAO_DE_REPARO' ? disabled : false,
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={isSubmitting} className="flex-1 rounded-none bg-warning py-2 font-medium text-warning-foreground hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-2">
